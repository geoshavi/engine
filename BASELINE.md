...
- OPEN ITEM (unresolved): whether defect classes such as misleading naming — which may structurally receive only MEDIUM/LOW severity under the current rubric, and therefore may not reach the severity needed to force an UNVERIFIED verdict — should be used as UNVERIFIED-expected benchmark cases at all. quality-03 is currently authored as exactly such a case (a naming-only difference: `get_user` vs `get_or_create_user`, everything else identical). If the answer is no, the rubric, the expected verdict, or the case itself has to change; this note records the question, not a decision.
