...
# --------------------------------------------------------------- Phase 8D.1
# Factual grounding: a blocking severity must be backed by a named triggering
# input. These lock the two halves of that contract -- that the judge actually
# ships the requirement to every lens, and that verdict.gate()'s treatment of
# severity is completely unchanged by it.


class _CapturingFakeProvider:
    """Records the exact user message each lens receives, then replays a
    scripted response. Lets a test assert on the prompt the judge really
    sent, rather than on the module constant in isolation.
    """
...
def _critic(*defects: dict) -> str:
    blocking = any(d["severity"] in ("CRITICAL", "HIGH") for d in defects)
    return json.dumps(
        {"defects": list(defects), "verdict": "FAIL" if blocking else "OK"}
    )


def _defect(severity: str, fix: str, category: str = "CORRECTNESS") -> dict:
    return {
        "id": "C1",
        "category": category,
        "severity": severity,
...
def _judge(responses: list[str]) -> tuple[list[dict], list[str], _CapturingFakeProvider]:
    provider = _CapturingFakeProvider(responses)
    critics, schema_errors = run_judge_gates(
        LLMGateway(provider),
        _budget(),
        "claude-haiku-4-5-20251001",
    ...
    return critics, schema_errors, provider


def _status(responses: list[str]) -> str:
    critics, schema_errors, _ = _judge(responses)
    return verdict.gate(verdict.merge(critics, []), True, schema_errors)


def test_every_lens_is_told_a_blocking_severity_needs_a_named_trigger() -> None:
    """The factual-grounding requirement must reach all three lenses, in the
    prompt actually transmitted -- not merely exist as a module constant.
    """
    _, _, provider = _judge([_critic(), _critic(), _critic()])

    assert len(provider.prompts) == 3
    for prompt in provider.prompts:
        lowered = prompt.lower()
        assert "critical or high" in lowered
        assert "concrete input or condition" in lowered
        assert "at most medium" in lowered


def test_the_trigger_requirement_does_not_soften_a_demonstrated_defect() -> None:
    """The rule is conditional, never a blanket downgrade: the instruction has
    to say so explicitly, or it reads as "be less strict".
    """
    _, _, provider = _judge([_critic(), _critic(), _critic()])

    assert "the severity it deserves" in provider.prompts[0]


def test_strict_tolerance_flagged_only_as_a_representation_preference_passes() -> None:
    """Positive, float/tolerance. The code satisfies an explicit strict-tolerance
    rule; the only finding is a preference for decimal arithmetic, with no input
    named at which the code returns the wrong answer. Non-blocking -> OK.
    """
    finding = _defect(
        "MEDIUM",
        "Binary floating point cannot represent 0.01 exactly, so decimal arithmetic "
        "would be a cleaner implementation. No input is named at which the supplied "
        "code returns the wrong answer for the stated rule.",
    )
    assert _status([_critic(finding), _critic(), _critic()]) == "OK"


def test_wrong_boundary_operator_still_blocks() -> None:
    """Negative counterexample, float/tolerance. The code uses <= where the task
    requires <, so a difference of exactly 0.01 is wrongly accepted. A named
    triggering input exists -> must still block.
    """
    finding = _defect(
        "HIGH",
        "a=1.00, b=1.01 differ by exactly the threshold and the code returns True, "
        "but the task states a difference of exactly the threshold is not close "
        "enough. Use < instead of <=.",
    )
    assert _status([_critic(finding), _critic(), _critic()]) == "UNVERIFIED"


def test_character_count_slicing_called_byte_slicing_does_not_block() -> None:
    """Positive, string semantics. Slicing a Unicode str by character count is
    reported without any input at which a character is actually corrupted --
    because the language slices code points, not bytes. Non-blocking -> OK.
    """
    finding = _defect(
        "LOW",
        "Slicing operates on code points, so no multi-byte sequence can be split; "
        "no input was found at which the supplied code corrupts a character.",
    )
    assert _status([_critic(), _critic(), _critic(finding)]) == "OK"


def test_encoded_byte_truncation_that_splits_a_character_still_blocks() -> None:
    """Negative counterexample, string semantics. Truncating the encoded bytes
    really can cut a multi-byte sequence in half. A named triggering input
    exists -> must still block.
    """
    finding = _defect(
        "HIGH",
        "text='cafe\u0301', max_chars=4 truncates the encoded bytes mid-sequence "
        "and raises UnicodeDecodeError. Slice the string, not its encoding.",
    )
    assert _status([_critic(finding), _critic(), _critic()]) == "UNVERIFIED"


def test_critical_still_blocks_regardless_of_how_the_finding_is_worded() -> None:
    finding = _defect("CRITICAL", "user input is interpolated straight into the query", "SECURITY")
    assert _status([_critic(), _critic(finding), _critic()]) == "UNVERIFIED"


def test_a_blocker_alongside_non_blocking_findings_still_blocks() -> None:
    preference = _defect("MEDIUM", "an alternate implementation would read better")
    real = _defect("HIGH", "page=1 returns the second page, which the task rules out")
    assert _status([_critic(preference, real), _critic(), _critic()]) == "UNVERIFIED"


def test_malformed_critic_response_still_fails_closed() -> None:
    assert _status(["not json at all", _critic(), _critic()]) == "UNVERIFIED"
